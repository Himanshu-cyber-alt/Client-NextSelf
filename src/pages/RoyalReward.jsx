...
export default function RoyalReward() {
  ...
  // Audio Refs (Lightweight HTML5 Audio)
  const claimTime = useRef(new Audio("/c.mp3"));
  const spin = useRef(new Audio("/d.mp3"));
  ...
  const runTimer = (startedAt, totalSeconds) => {
    ...
    const interval = setInterval(() => {
      ...
      if (remaining <= 0) {
        ...
        // PLAY ALARM directly via HTML5
        if (alarmAudio.current) {
          alarmAudio.current.loop = true;
          alarmAudio.current.play().catch(e => console.log("Alarm blocked by browser:", e));
        }
      } else {
        setTimeLeft(remaining);
      }
  ...
  const startTimer = async () => {
    ...
    try {
      ...
      // Quick trick to unlock the alarm audio element on mobile so it can play later
  ...
  // Resume a timer already running
  useEffect(() => {
    if (!uuid) return;
    const saved = localStorage.getItem(`timer-${uuid}`);
    if (!saved) return;
  ...
  // Stop alarm function (Optional but highly recommended if they want to turn off the ringing!)
      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center justify-center gap-6 w-full h-screen px-6">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col items-center justify-center h-full">
            ...
            {/* If time is 0, show consume, otherwise show Stop Alarm if ringing */}
            {timeLeft <= 0 ? (
                <button
                onClick={startTimer}
                disabled={playing || rewardMinutes <= 0}
                ...
                className="mt-12 w-full rounded-2xl bg-black hover:bg-gray-800 border-2 border-gray-700 py-4 font-semibold text-white transition"
                >
                Stop Alarm
                </button>
            )}
