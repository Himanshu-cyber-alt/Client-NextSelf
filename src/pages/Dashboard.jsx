...
const POLL_INTERVAL = 5000;
const DURATION = 45 * 60; // 45 minutes for production
...
export default function Dashboard() {
  ...
  // ---------------------------------------------------------------------------
  // 1. PRELOAD AUDIO INSTANTLY
  // ---------------------------------------------------------------------------
  useEffect(() => {
    // This forces the browser to download sounds immediately upon opening the page
    // so there is zero delay when you click the buttons.
    ...
    claimAudio.current.preload = "auto";

    keepAliveAudio.current = new Audio("/silent-loop.mp3");
    keepAliveAudio.current.preload = "auto";
  }, []);
  ...
  // ---------------------------------------------------------------------------
  // 3. GLOBAL TIMER LOGIC (No backend calls here!)
  // ---------------------------------------------------------------------------
  const runGlobalTimer = (startedAt, taskId) => {
    if (timerRef.current) clearInterval(timerRef.current);
  ...
  // ---------------------------------------------------------------------------
  // 4. START TASK (With Auto-Recovery)
  // ---------------------------------------------------------------------------
  const handleStartTask = useCallback(async (task) => {
    if (activeTaskId) return; 
    ...
    try {
      ...
      // 1. Play Preloaded Audio Instantly
  ...
  // ---------------------------------------------------------------------------
  // 5. STOP & CLAIM (With Offline Protection)
  // ---------------------------------------------------------------------------
  const handleStopAlarm = useCallback(async (task) => {
    // 1. Prevent action if completely offline
    if (!navigator.onLine) {
    ...
    try {
      ...
      // Play preloaded claim sound instantly
